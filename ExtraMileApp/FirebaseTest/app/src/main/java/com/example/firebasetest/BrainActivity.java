...
//        info.put("time",time);
//        database.collection("bobthbuilder").document("user"+String.valueOf(counter+1))
...
//    }
    ...
    private final INeuosSdkListener mCallback = new INeuosSdkListener.Stub() {
        ...
        public void onValueChanged(String key, float value) throws RemoteException {
            ...
            info.put(key, value);
            if (counter%25==0){
                database.collection("readings").add(info).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {@Override
                public void onSuccess(DocumentReference documentReference) {
                    Log.i(TAG,"Updated Data in Firebase");
                }});
            }
